...
test("detail carries a live fair-value estimate + deal score + collection meta", () => {
  ...
  assert.equal(mapped.nft.fairValue!.totalEstimate, 1.252295); // floor 1.0 + rarity .222295 + desirability .03
...
test("a resolved floor override (e.g. Dexie) replaces MintGarden's floor in valuation", () => {
  ...
  // rarity premium scales with floor: 2.0 * 0.22229466... = 0.444589 (6dp, not the old 2dp 0.44)
  assert.equal(nft.fairValue?.rarityPremium, 0.444589);
});
